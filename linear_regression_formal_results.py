...
print(f"Target variable: sales quantity")
...
that predictions deviate from actual sales by approximately {mae:.2f} units on
...
performed, yielding a mean MAE of {cv_mean:.4f} ± {cv_std:.4f}. The minimal
difference between training MAE ({train_mae:.4f}) and test MAE ({test_mae:.4f}),
...
{example_pred:.0f} units of sales for a given day, the actual sales will likely
fall between {example_low:.0f} and {example_high:.0f} units, enabling precise
...
""".format(
    ...
    coef2=model.coef_[2],
    example_pred=20,
    example_low=20-test_mae,
    example_high=20+test_mae,
...
Five-fold cross-validation yielded {cv_mean:.4f} ± {cv_std:.4f} MAE, confirming
...
print(f"Cross-Validation MAE & {cv_mae_mean:.4f} $\\pm$ {cv_mae_std:.4f} \\\\")
